...
| B8 | **Aucun appel réel à un fournisseur d'intégration.** Les sept connecteurs *et désormais le flux OAuth* sont écrits contre des documentations | 🔴 Haute | Même blocage que A1. Il faut s'attendre à ce que plusieurs adaptateurs soient faux — ADR-057 en assume le coût. Le flux OAuth est testé contre un serveur d'autorisation simulé (ADR-064) |
