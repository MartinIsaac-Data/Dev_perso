"""Note extraction — the phase 1 prompt, moved here unchanged (AI.md §4.3).

The text is byte-identical to what shipped in phase 1. It is *moved*, not
rewritten: `app.infra.ai.prompts` re-exports every name it used to define, so
existing callers and the stored fingerprints on historic `ai_run` rows both stay
valid. Changing the text here would change the fingerprint and silently orphan
that history, which is why this module gained a version number and not an edit.

Layout follows the caching order of AI.md §4.3: stable system instructions
first, semi-stable user context next, volatile transcript last. Putting the
timestamp in the system block would invalidate the prefix cache on every call —
...
EXTRACTION_PROMPT = register(
    Prompt(
        name=PROMPT_NAME,
        version=PROMPT_VERSION,
        system=SYSTEM_PROMPT,
        description="Transcription vocale → NoteAnalysis structurée.",
        tags=("capture", "structured-output"),
    )
)
...
def prompt_fingerprint() -> str:
    ...
    return EXTRACTION_PROMPT.fingerprint
